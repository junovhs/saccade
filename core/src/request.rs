...
impl RequestFile {
    ...
    pub fn resolve(&self, available_files: &[PathBuf]) -> Result<ResolvedRequest> {
        ...
        let files = matching_paths
            .into_iter()
            .filter_map(|path| self.read_file_with_range(&path).ok())
            .collect();
    ...
    fn read_file_with_range(&self, path: &Path) -> Result<FileContent> {
    ...
    fn extract_line_range(&self, content: &str, range_spec: &str) -> Result<(String, String)> {
        ...
        let (start, end) = if let Some((start_str, end_str)) = range_spec.split_once('-') {
            ...
            // Single line: "80"
            let line = range_spec.trim().parse::<usize>()
                .map_err(|_| RequestError::InvalidLineRange(range_spec.to_string()))?;
            (line, line)
        };
...
impl ResolvedRequest {
    ...
    pub fn to_markdown(&self) -> String {
        ...
        output.push_str(&format!("# REQUEST_FILE Results\n\n"));
...
"#;
    ...
    fn test_single_path_request() {
        ...
        let resolved = request.resolve(&files).unwrap();
    ...
    fn test_file_not_found() {
        ...
        assert!(request.resolve(&files).is_err());
    }
